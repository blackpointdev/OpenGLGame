...
static unsigned int CreateShader(const std::string& vertexShader, const std::string& fragemntShader)
{
	GLCall(unsigned int program = glCreateProgram());
	unsigned int vs = CompileShader(GL_VERTEX_SHADER, vertexShader);
	unsigned int fs = CompileShader(GL_FRAGMENT_SHADER, fragemntShader);
...
int main(void)
{
	...
	// Basic settings of OpenGL such as version and profile
	...
	// Set refresh rate (FPS basicly) to 60
	glfwSwapInterval(1);
